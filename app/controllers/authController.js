...
exports.adminLogin = [
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        logger.warn(
          `Admin login attempt failed: Admin not found for username ${username}`
        );
      ...
      if (!isMatch) {
        logger.warn(
          `Admin login attempt failed: Invalid credentials for username ${username}`
        );
...
exports.logout = (req, res) => {
  req.session.destroy((err) => {
    ...
    const redirectUrl =
      req.session?.user?.role === "admin" ? "/auth/admin-login" : "/auth/login";
